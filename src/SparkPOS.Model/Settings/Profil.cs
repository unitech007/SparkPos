...
{        
    ...
    public class ProfilValidator : AbstractValidator<Profil>
    {
        public ProfilValidator()
        {
			...
			RuleFor(c => c.name_profile).NotEmpty().WithMessage(msgError1).Length(1, 100).WithMessage(msgError2);
			RuleFor(c => c.address).NotEmpty().WithMessage(msgError1).Length(1, 100).WithMessage(msgError2);
