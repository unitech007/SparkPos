...
namespace SparkPOS.Model.Transaction
{
    ...
    [Table("t_sales_quotation_item")]
    public class ItemSellingQuotation
    {
        public ItemSellingQuotation()
        {
            entity_state = EntityState.Added;
        }

        [ExplicitKey]
        [Display(Name = "quotation_item_id")]
    ...
    public class ItemSellingQuotationValidator : AbstractValidator<ItemSellingQuotation>
    {
        public ItemSellingQuotationValidator()
        {
            ...
            RuleFor(c => c.quotation_id).NotEmpty().WithMessage(msgError1).Length(1, 36).WithMessage(msgError2);
