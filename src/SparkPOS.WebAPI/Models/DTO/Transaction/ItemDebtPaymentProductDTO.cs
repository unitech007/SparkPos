...
{
    public class ItemDebtPaymentProductDTO
    {
        ...
        [JsonIgnore]		
        public DebtPaymentProductDTO DebtPaymentProduct { get; set; }
    ...
    public class ItemDebtPaymentProductDTOValidator : AbstractValidator<ItemDebtPaymentProductDTO>
    {
        public ItemDebtPaymentProductDTOValidator()
        {
            ...
            RuleSet("update", () =>
            {
                RuleFor(c => c.pay_purchase_item_id).NotEmpty().WithMessage(msgError1).Length(1, 36).WithMessage(msgError2);
                DefaultRule(msgError1, msgError2);
            });
