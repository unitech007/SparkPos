    ...
    public partial class FrmMain : Form, IListener
    {
        ...
        public void InitializeStatusBar()
        {
            ...
            if (MainProgram.currentLanguage == "en-US")
            {
                ...
                MainProgram.AppName = "Spark POS Version {0}{1} - Copyright © {2}";
            }
            else if (MainProgram.currentLanguage == "ar-SA")
            {
                if (MainProgram.user != null)
                    sbOperator.Text = string.Format("عامل التشغيل: {0}", MainProgram.user.name_user);
                MainProgram.AppName = "نسخة Spark POS {0}{1} - حقوق النشر © {2}";
            }
            ...
            //ProcessLabel(sbOperator, rm, languageToLoad);
        ...
        //    if (MainProgram.user != null)
        //    {
        //        if (MainProgram.currentLanguage == "en-SA")
        //        {
        //            sbOperator.Text = LanguageHelper.TranslateText(string.Format("Operator : {0}"), MainProgram.currentLanguage);
        //        }
        //        else if (MainProgram.currentLanguage == "ar-SA")
        //        {
        //            sbOperator.Text = LanguageHelper.TranslateText(string.Format("Operator : {0}"), MainProgram.currentLanguage);
        //            sbNamaApplication.Text = LanguageHelper.TranslateText(appName.Replace("&", "&&"), MainProgram.currentLanguage);
        //        }
        //        else
        //        {
        //            sbOperator.Text = string.Format("Operator : {0}", MainProgram.user.name_user);
        ...
        //{
        //    var header = GetMenuTitle(sender);
        //    var formName = GetFormName(sender);

        //    if (_getMenuID.TryGetValue(formName, out var menuId))
        //    {
        //        if (!IsChildFormExists((DockContent)(object)form))
        //            form = (T)Activator.CreateInstance(typeof(T), header, MainProgram.user, menuId);
        ...
        //        // You can provide a default or fallback value for menuId or perform any other necessary action
        //        // For example, you can show an error message or log the issue
        //        MessageBox.Show("Menu ID not found for the specified form.");
        //    }
        //}
        ...
        private void ShowFormDialog<T>(object sender)
        {
            ...
            if (menuName.Substring(0, 6) == "mnuLap")
            {
                ...
                {
                    header = string.Format("تقرير {0}", GetMenuTitle(sender));
                }
            ...
            {
                string msg = "Sorry you do not have the authority to access this menu";
                WarningMessageHandler.ShowTranslatedWarning(msg, MainProgram.currentLanguage);
            }
        ...
        private void mnuGeneralSupplier_Click(object sender, EventArgs e)
        {
            ...
            if (RolePrivilegeHelper.IsHaveRightAccess(menuName, MainProgram.user, GrantState.UPDATE))
            {
                ...
                WarningMessageHandler.ShowTranslatedWarning("Sorry, you do not have the authority to access this menu", MainProgram.currentLanguage);
           // WarningMessageHandler.ShowTranslatedWarning(msg, MainProgram.currentLanguage);
        }
        ...
        private void mnuCheckUpdateLatest_Click(object sender, EventArgs e)
        {
            if (MainProgram.onlineUpdateUrlInfo.Length > 0)
            {
                ...
                MsgHelper.MsgWarning("Key-OnlineUpdate");
        }
       ...
       // deliveryNotesToolStripMenuItem_Click
        ...
        private void languageToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (MsgHelper.MsgConfirmation("Continuing this will log you out and any unsaved changes will be lost. Save your changes before proceeding. Are you sure you want to continue?"))
            {
                using (new StCursor(Cursors.WaitCursor, new TimeSpan(0, 0, 0, 0)))
                {
                    AutoUpdater.CheckForUpdateEvent -= AutoUpdaterOnCheckForUpdateEvent;
