...
using SparkPOS.Bll.Service.Transaction;
using MultilingualApp;

namespace SparkPOS.App.Transactions
{
    public partial class FrmListSalesQuotationWithNavigation : FrmListEmptyBodyWithNavigation, IListener
    {
        private ISellingQuotationBll _bll; // deklarasi objek business logic layer 
        private IList<SellingQuotation> _listOfSelling = new List<SellingQuotation>();
        ...
        public FrmListSalesQuotationWithNavigation(string header, User user, string menuId)
            : base()
        {
             InitializeComponent();  
           ...
           _bll = new SellingQuotationBll(MainProgram.isUseWebAPI, MainProgram.baseUrl, _log);
            _user = user;
        ...
        private void InitGridList()
        {
            ...
            gridListProperties.Add(new GridListControlProperties { Header = "No", Width = 40 });
            gridListProperties.Add(new GridListControlProperties { Header = "date", Width = 120 });
            gridListProperties.Add(new GridListControlProperties { Header = "Quotation", Width = 130 });
            ...
            gridListProperties.Add(new GridListControlProperties { Header = "Description", Width = 400 });

            if (_GeneralSupplier.type_printer == TypePrinter.InkJet)
            {
                gridListProperties.Add(new GridListControlProperties { Header = "Print Quotation/Label", Width = 120 });
                gridListProperties.Add(new GridListControlProperties { Header = "" });
            }
            else
            {
                gridListProperties.Add(new GridListControlProperties { Header = "Print Quotation", Width = 110 });
            }

            //if (_GeneralSupplier.type_printer == TypePrinter.InkJet)
            //{
            //    gridListProperties.Add(new GridListControlProperties { Header = "Print Quotation/Label", Width = 80 });
            //    gridListProperties.Add(new GridListControlProperties { Header = "" });
            //}
            //else
            //{
            //    gridListProperties.Add(new GridListControlProperties { Header = "Print Quotation", Width = 80 });
            //}
            LanguageHelper.TranslateGridListControlHeaders(gridListProperties, MainProgram.currentLanguage);
            GridListControlHelper.InitializeGridListControl<SellingQuotation>(this.gridList, _listOfSelling, gridListProperties, false, rowHeight: 40);
                ...
                if (e.RowIndex > 0)
                {
                    ...
                    switch (e.ColIndex)
                    {
                        case 6: // history payment
                            ...
                            using (new StCursor(Cursors.WaitCursor, new TimeSpan(0, 0, 0, 0)))
                            {
                                ...
                                switch (this._GeneralSupplier.type_printer)
                                {
                                    ...
                                    default:
                                        var frmPrintInvoice = new Transaction.FrmPreviewInvoiceSalesQuotation("Preview Invoice Sales Quotation", sale);

                                        if (MainProgram.currentLanguage == "en-US")
                                        {
                                             frmPrintInvoice = new Transaction.FrmPreviewInvoiceSalesQuotation("Preview Invoice Sales Quotation", sale);

                                        }
                                        else if (MainProgram.currentLanguage == "ar-SA")
                                        {
                                             frmPrintInvoice = new Transaction.FrmPreviewInvoiceSalesQuotation("معاينة عرض أسعار مبيعات الفاتورة", sale);
                        ...
                        case 10: // print invoice sale
                            ...
                            //        default:
                            //            var frmPrintInvoice = new Transaction.FrmPreviewInvoiceSalesQuotation("Preview Invoice Sales", sale);
                            //            frmPrintInvoice.ShowDialog();
                            //            break;
                            //    }
                            //}
                ...
                if (_listOfSelling.Count > 0)
                {
                    if (e.RowIndex > 0)
                    {
                        ...
                        if (rowIndex < _listOfSelling.Count)
                        {
                            ...
                            if (sale != null)
                               // totalQuotation = sale.grand_total;
                            ...
                            switch (e.ColIndex)
                            {
                                ...
                                case 3:
                                        ...
                                        e.Style.CellValue = sale.quotation;
                                        break;
                                ...
                                case 6:
                                        ...
                                        if (MainProgram.currentLanguage == "en-US")
                                        {
                                            e.Style.Description = "Print Quotation";
                                        }
                                        else if (MainProgram.currentLanguage == "ar-SA")
                                        {
                                            e.Style.Description = "اقتباس طباعة";
                                        }
                                ...
                                case 7:
                                        //e.Style.HorizontalAlignment = GridHorizontalAlignment.Right;
                                        //e.Style.CellValue = NumberHelper.NumberToString(totalQuotation);
                                        //e.Style.Enabled = sale.Customer != null;
                                        //e.Style.HorizontalAlignment = GridHorizontalAlignment.Center;
                                        //e.Style.CellType = GridCellTypeName.PushButton;
                                        ////e.Style.BackColor = oldStyleBackColor;
                                ...
                                //case 8:
                                //        //e.Style.HorizontalAlignment = GridHorizontalAlignment.Right;
                                //        //e.Style.CellValue = NumberHelper.NumberToString(totalQuotation - sale.total_payment);
                                //        if (_GeneralSupplier.type_printer == TypePrinter.InkJet)
                                //        {
                                //            e.Style.Enabled = sale.Customer != null;
                                //            e.Style.HorizontalAlignment = GridHorizontalAlignment.Center;
        ...
        private void PrintInvoiceMiniPOS(SellingQuotation sale)
        {
        ...
        private void PrintInvoiceDotMatrix(SellingQuotation sale)
        {
            IRAWPrinting printerDotMatrix = new PrinterDotMatrix(_GeneralSupplier.name_printer);
            printerDotMatrix.Print(sale, _GeneralSupplier.list_of_header_nota, _GeneralSupplier.jumlah_gulung, isPrintKeteranganInvoice: _GeneralSupplier.is_print_keterangan_nota);
        }
        ...
        private void LoadData()
        {
            using (new StCursor(Cursors.WaitCursor, new TimeSpan(0, 0, 0, 0)))
            {
                ...
                GridListControlHelper.Refresh<SellingQuotation>(this.gridList, _listOfSelling);
        ...
        protected override void Add()
        {
            using (new StCursor(Cursors.WaitCursor, new TimeSpan(0, 0, 0, 0)))
            {
                ...
                var frm = new FrmEntrySalesQuotation(formTitle + this.Text, _bll);
                frm.Listener = this;
                frm.ShowDialog();
            }            
        }
        ...
        protected override void Edit()
        {
            ...
            using (new StCursor(Cursors.WaitCursor, new TimeSpan(0, 0, 0, 0)))
            {
                ...
                sale.item_jual = _bll.GetItemSelling(sale.quotation_id).ToList();
                ...
                var frm = new FrmEntrySalesQuotation(formTitle + this.Text, sale, _bll);
                frm.Listener = this;
                frm.ShowDialog();
            }
        }
        ...
        protected override void Delete()
        {
            ...
            if (MsgHelper.MsgDelete())
            {
                using (new StCursor(Cursors.WaitCursor, new TimeSpan(0, 0, 0, 0)))
                {
                    ...
                    if (result > 0)
                    {
                        GridListControlHelper.RemoveObject<SellingQuotation>(this.gridList, _listOfSelling, sale);
                        ResetButton();
                    }
                    else
        ...
        {
            var sale = (SellingQuotation)data;

            if (isNewData)
            {
                GridListControlHelper.AddObject<SellingQuotation>(this.gridList, _listOfSelling, sale);
                ResetButton();
            }
            else
                GridListControlHelper.UpdateObject<SellingQuotation>(this.gridList, _listOfSelling, sale);
        }
