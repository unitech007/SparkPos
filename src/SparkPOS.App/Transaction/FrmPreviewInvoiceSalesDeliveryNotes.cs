...
{
    public partial class FrmPreviewInvoiceSalesDeliveryNotes : Form 
    {
        ...
        public FrmPreviewInvoiceSalesDeliveryNotes()
        {
             InitializeComponent();  
        ...
        public FrmPreviewInvoiceSalesDeliveryNotes(string header, SellingDeliveryNotes sale)
            : this()
        {
        ...
        //private void PreviewInvoice(SellingDeliveryNotes sale, bool isPreview = true)
        //{
        //    try
        //    {
        //        IPrintDeliveryNotesBll printBll = new PrintInvoiceBll(_log);
        ...
        //public static void lblHeader1(FrmPreviewInvoiceSalesDeliveryNotes frm)
        //{
        //    if (MainProgram.currentLanguage == "en-US")
        //    {
        //       frm.lblHeader.Text =  "Preview Invoice Delivery Notes";

        //    }
        //    else if (MainProgram.currentLanguage == "ar-SA")
        //    {
        //        frm.lblHeader.Text =  "معاينة ملاحظات تسليم الفاتورة";
        ...
        private void PreviewInvoice(SellingDeliveryNotes sale, bool isPreview = true)
        {
            try
            {
                PrintSellingDeliveryNotesBll printBll = new PrintSellingDeliveryNotesBll(_log);
                var listOfItemInvoice = printBll.GetSellingDeliveryNotes(sale.delivery_id);
                ...
                if (listOfItemInvoice.Count > 0)
                {
                    ...
                    var reportDataSource = new ReportDataSource
                    {
                        Name = "DeliveryNotesSales",
                        Value = listOfItemInvoice
                    };

                    //string code = "Your information you want to display in the QR code";

                    //  string qrCodeData = $"{listOfItemInvoice.} - Price: {listOfItemInvoice.Price} - Quantity: {listOfItemInvoice.Quantity}";
                    string code = "";
                    foreach (var item in listOfItemInvoice)
                    {
                        string productInfo = $"{item.product_name} - Price: {item.price} - Quantity: {item.quantity}";
                        code += productInfo + "\n";
                    }
       ...
       "QrCode_" + DateTime.Now.ToString("yyyyMMddHHmmssfff") + ".png"
                  ...
                  //  MessageBox.Show(savePath);
                    ...
                    parameters.Add(new ReportParameter("QR_Img", qrCodeImagePath, true));
                                      //  var reportName = sale.is_dropship ? "RvInvoiceSalesQuotationTanpaLabel" : "RvInvoiceSalesQuotationTanpaLabel";
                                        var reportName = sale.is_dropship ? "RvInvoiceSalesDeliveryNotesTanpaLabel" : "RvInvoiceSalesDeliveryNotesTanpaLabel";
                   // var reportName = sale.is_dropship ? "Report1" : "Report1";
        ...
        private void btnPrintInvoice_Click(object sender, EventArgs e)
        {
            if (MsgHelper.MsgConfirmation("Do you want to continue the printing process ?"))
            {
                using (new StCursor(Cursors.WaitCursor, new TimeSpan(0, 0, 0, 0)))
                {
                    ISellingDeliveryNotesBll bll = new SellingDeliveryNotesBll(_log);
        ...
        private void FrmPreviewInvoiceSalesDeliveryNotes_KeyPress(object sender, KeyPressEventArgs e)
        {
        ...
        private void FrmPreviewInvoiceSalesDeliveryNotes_Load(object sender, EventArgs e)
        {
