...
{
    public partial class FrmEntrySalesDeliveryNotes : FrmEntryStandard, IListener
    {
        private ISellingDeliveryNotesBll _bll = null;
        private SellingDeliveryNotes _jual = null;
        private Customer _customer = null;
        private Dropshipper _dropshipper = null;
        private IList<ItemSellingDeliveryNotes> _listOfItemSelling = new List<ItemSellingDeliveryNotes>();
        private IList<ItemSellingDeliveryNotes> _listOfItemSellingOld = new List<ItemSellingDeliveryNotes>();
        private IList<ItemSellingDeliveryNotes> _listOfItemSellingDeleted = new List<ItemSellingDeliveryNotes>();
        ...
        public FrmEntrySalesDeliveryNotes(string header, ISellingDeliveryNotesBll bll) 
            : base()
        {            
             InitializeComponent();  
            ...
            txtdelivery.Text = bll.GetLastDeliveryNotes();
            ...
            _listOfItemSelling.Add(new ItemSellingDeliveryNotes()); // add dummy objek
        ...
        public FrmEntrySalesDeliveryNotes(string header, SellingDeliveryNotes sale, ISellingDeliveryNotesBll bll)
            : base()
        {
             InitializeComponent(); 
            ColorManagerHelper.SetTheme(this, this);
            ...
            txtdelivery.Text = this._jual.delivery;
            dtpDate.Value = (DateTime)this._jual.delivery_date;
           //dtpDateCreditTerm.Value = dtpDate.Value;
            btnPreviewInvoice.Visible = _GeneralSupplier.type_printer == TypePrinter.InkJet;
        ...
        // txtKeterangan.Text = this._jual.description;
            ...
            {
                _listOfItemSellingOld.Add(new ItemSellingDeliveryNotes
                {
        ...
        private void LoadInvoice(string customerId)
        {
            ...
            List<string> quotations = this._bll.GetInvoiceByCustomerId(customerId);

            cmbInvoice.Items.Clear();
            cmbInvoice.Items.Add("Select");
            cmbInvoice.Items.AddRange(quotations.ToArray());
        ...
        private void InitGridControl(GridControl grid)
        {
            ...
            GridListControlHelper.InitializeGridListControl<ItemSellingDeliveryNotes>(grid, _listOfItemSelling, gridListProperties);
        ...
        private double SumGrid(IList<ItemSellingDeliveryNotes> listOfItemSelling)
        {
            double total = 0;
      ...
      //      }
            ...
            {
                if (this._jual == null)
                    _jual = new SellingDeliveryNotes();
            }
            ...
            _jual.delivery = txtdelivery.Text;
            _jual.delivery_date = dtpDate.Value;
                        ...
                        if (chkPrintInvoiceSelling.Checked)
                        {
                            switch (this._GeneralSupplier.type_printer)
                            {
                                ...
                                case TypePrinter.MiniPOS:
                                    ...
                                    break;
                                default:
                                    PrintInvoice(_jual.delivery_id);
                                    break;
                            }
                        }
                    }
        ...
        {
            IPrintSellingDeliveryNotesBll printBll = new PrintSellingDeliveryNotesBll(_log);
            var listOfItemInvoice = printBll.GetSellingDeliveryNotes(jualProductId);
        ...
        private void PrintInvoiceMiniPOS(SellingDeliveryNotes sale)
        {
        ...
        private void PrintInvoiceDotMatrix(SellingDeliveryNotes sale)
        {
        ...
        //  //        ItemSellingDeliveryNotes itemSelling = null;
        ...
        //  //            {
        //  //                _listOfItemSelling.Add(new ItemSellingDeliveryNotes());
        ...
        //  //        if (this._jual == null)
        //  //            this._jual = new SellingDeliveryNotes();
        ...
        private void txtCustomer_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (KeyPressHelper.IsEnter(e))
            {
                ...
                else if (listOfCustomer.Count == 1)
                {
                    ...
                    LoadInvoice(_customer.customer_id);
                    KeyPressHelper.NextFocus();
        ...
        {
            ItemSellingDeliveryNotes itemSelling;

            if (_isNewData)
            {
                itemSelling = new ItemSellingDeliveryNotes();
            }
            else
            {
        ...
        private ItemSellingDeliveryNotes GetExistItemProduct(string produkId)
        {
            ...
            if (KeyPressHelper.IsEnter(e))
            {
                ...
                switch (colIndex)
                {
                    ...
                    //       var isValidStock = (product.remaining_stock + itemSelling.old_jumlah - itemSelling.quantity) >= 0;
                            ...
                            //               "selling quantity: {1}\n" +
                            //               "Remaining stock: {2}";
                            //    MsgHelper.MsgWarning((msg, product.remaining_stock.ToString(), itemSelling.quantity, product.remaining_stock.ToString() + itemSelling.old_jumlah - itemSelling.quantity));
                            //    GridListControlHelper.SelectCellText(grid, rowIndex, colIndex);
        ...
        private void PreviewInvoice()
        {
            ...
            var listOfItemInvoice = new List<QuotationSales>();

            foreach (var item in this._listOfItemSelling.Where(f => f.Product != null))
            {
                var itemInvoice = new QuotationSales
                {
            ...
            var frmPreviewReport = new FrmPreviewReport("Preview Invoice Delivery Notes", reportName, reportDataSource, parameters, true);
            if (MainProgram.currentLanguage == "en-US")
            {
                 frmPreviewReport = new FrmPreviewReport("Preview Invoice Delivery Notes", reportName, reportDataSource, parameters, true);
            }
            else if (MainProgram.currentLanguage == "ar-SA")
            {
                 frmPreviewReport = new FrmPreviewReport("معاينة ملاحظات تسليم الفاتورة", reportName, reportDataSource, parameters, true);
            }
        ...
        private void cmbKurir_SelectedIndexChanged(object sender, EventArgs e)
        {
        ...
        {
            if (cmbInvoice.SelectedItem != null)
            {
                string selectedQuotation = cmbInvoice.SelectedItem.ToString();

                // Call the GetProductDetailsByQUotation method and receive the list of product details
                List<ItemSellingProduct> productDetails = _bll.GetProductDetailsByInvoice(selectedQuotation);
                ...
                foreach (var productDetail in productDetails)
                {
                    _listOfItemSelling.Add(new ItemSellingDeliveryNotes
                    {
                       ...
                       selling_price= productDetail.selling_price,
                        ...
                        //f selling_price = productDetail.selling_price,
                ...
                // Load the description into the textbox
                if (productDetails.Count > 0)
                {
                    txtKeterangan.Text = productDetails[0].description;
                }
            }
        }
