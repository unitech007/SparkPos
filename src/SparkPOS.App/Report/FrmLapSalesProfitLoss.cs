...
{
    public partial class FrmLapSalesProfitLoss : FrmSettingReportEmptyBody
    {
        private ILog _log;

        public FrmLapSalesProfitLoss(string header)
        {
             InitializeComponent();  
        ...
        private void PreviewReport()
        {
            ...
            IReportLossProfitBll reportBll = new ReportLossProfitBll(_log);
            ReportLossProfit reportProfitLoss = null;
            ...
            if (rdoDate.Checked)
            {
                ...
                reportProfitLoss = reportBll.GetByDate(dtpDateMulai.Value, dtpDateSelesai.Value);
            }
            else
            {
                ...
                reportProfitLoss = reportBll.GetByMonth(month, year);
            }

            if (reportProfitLoss != null)
            {
                var reportDataSource = new ReportDataSource
                {
                    Name = "DsReportLossProfit",
                    Value = new List<ReportLossProfit> { reportProfitLoss }
                };
                ...
                base.ShowReport(this.Text, "RvLossProfit", reportDataSource, parameters);
            }            
            else
            {
                MsgHelper.MsgInfo("Sorry report data loss profit not found");
            }
        }
    }
}
