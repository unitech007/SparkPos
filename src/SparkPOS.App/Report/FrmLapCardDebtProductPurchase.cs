...
{
    public partial class FrmLapCardDebtProductPurchase : FrmSettingReportStandard
    {
        private IList<Supplier> _listOfSupplier = new List<Supplier>();
        private ILog _log;

        public FrmLapCardDebtProductPurchase(string header)
        {
             InitializeComponent();  
        ...
        private void PreviewReport()
        {
            ...
            IReportCardDebtBll reportBll = new ReportCardDebtBll(_log);
            
            IList<ReportCardDebt> listOfReportCardDebt = new List<ReportCardDebt>();
            ...
            if (rdoDate.Checked)
            {
                ...
                listOfReportCardDebt = reportBll.GetByDate(dtpDateMulai.Value, dtpDateSelesai.Value);
            }
            else
            {
                ...
                listOfReportCardDebt = reportBll.GetByMonth(month, year);
            }

            if (listOfSupplierId.Count > 0 && listOfReportCardDebt.Count > 0)
            {
                listOfReportCardDebt = listOfReportCardDebt.Where(f => listOfSupplierId.Contains(f.supplier_id))
                                                             .ToList();
            }

            if (listOfReportCardDebt.Count > 0)
            {
                var reportDataSource = new ReportDataSource
                {
                    Name = "ReportCardDebtProductPurchase",
                    Value = listOfReportCardDebt
                };
                ...
                base.ShowReport(this.Text, "RvCardDebtProductPurchase", reportDataSource, parameters);
            }
            else
            {
                MsgHelper.MsgInfo("Sorry, the data card debt purchase report was not found");
            }
        }
    }
}
