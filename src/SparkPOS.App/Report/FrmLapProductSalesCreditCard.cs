...
{
    public partial class FrmLapProductSalesCreditCard : FrmSettingReportStandard
    {
        private IList<Customer> _listOfCustomer = new List<Customer>();
        private ILog _log;

        public FrmLapProductSalesCreditCard(string header)
        {
             InitializeComponent(); 
        ...
        private void PreviewReport()
        {
            ...
            IReportCardCreditBll reportBll = new ReportCardCreditBll(_log);

            IList<ReportCardCredit> listOfReportCardCredit = new List<ReportCardCredit>();
            ...
            if (rdoDate.Checked)
            {
                ...
                listOfReportCardCredit = reportBll.GetByDate(dtpDateMulai.Value, dtpDateSelesai.Value);
            }
            else
            {
                ...
                listOfReportCardCredit = reportBll.GetByMonth(month, year);
            }

            if (listOfCustomerId.Count > 0 && listOfReportCardCredit.Count > 0)
            {
                listOfReportCardCredit = listOfReportCardCredit.Where(f => listOfCustomerId.Contains(f.customer_id))
                                                             .ToList();
            }

            if (listOfReportCardCredit.Count > 0)
            {
                var reportDataSource = new ReportDataSource
                {
                    Name = "ReportCardCreditSalesProduct",
                    Value = listOfReportCardCredit
                };
                ...
                base.ShowReport(this.Text, "RvCardCreditSalesProduct", reportDataSource, parameters);
            }
            else
            {
                MsgHelper.MsgInfo("Sorry report data Credit Card sales not found");
            }
        }
    }
}
