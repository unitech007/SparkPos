...
{
    public class PrintInvoiceBll : IPrintInvoiceBll
    {
        private ILog _log;
        private IUnitOfWork _unitOfWork;

        public PrintInvoiceBll(ILog log)
        {
            _log = log;
        }
        ...
        public IList<InvoicePurchase> GetInvoicePurchase(string beliProductId)
        {
            ...
            using (IDapperContext context = new DapperContext())
            {
                ...
                oList = _unitOfWork.PrintInvoiceRepository.GetInvoicePurchase(beliProductId);
            }
        ...
        public IList<InvoiceSales> GetInvoiceSales(string jualProductId)
        {
            ...
            {
                _unitOfWork = new UnitOfWork(context, _log);
                oList = _unitOfWork.PrintInvoiceRepository.GetInvoiceSales(jualProductId);
            }
            ...
            foreach (var item in oList)
            {
                ...
                item.cr_no = string.IsNullOrEmpty(item.cr_no) ? "" : item.cr_no;
                item.vat_no = string.IsNullOrEmpty(item.vat_no)? "" : item.vat_no;
